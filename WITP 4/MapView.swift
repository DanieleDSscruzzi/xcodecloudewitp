                    ...
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(spot.streetName)
                                ...
                                .lineLimit(1)
                            Text("\(Int(spot.distanceFromUser)) m · \(spot.stallCount > 0 ? "≈\(spot.stallCount) stalli" : "capienza n/d") · \(spot.zoneType.label)")
                                .font(.caption)
...
private struct AnswerCard: View {
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 6) {
                Text("\(Int(spot.availability * 100))%")
                    ...
                    .foregroundStyle(spot.availabilityLevel.color)
                Text("stima · \(Int(spot.distanceFromUser)) m · \(spot.stallCount > 0 ? "≈\(spot.stallCount) stalli" : "capienza n/d") · \(spot.zoneType.label)")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
            ...
            HStack {
                ...
                ShareLink(item: shareURL,
                          subject: Text("Parcheggio — \(spot.streetName)"),
                          message: Text("Trovato con WITP: \(spot.streetName)")) {
                    Image(systemName: "square.and.arrow.up")
                }
                .accessibilityLabel("Condividi parcheggio")
    ...
    /// Link Apple Maps del posto: chi lo riceve apre le indicazioni.
    private var shareURL: URL {
        let q = spot.streetName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "Parcheggio"
        return URL(string: "https://maps.apple.com/?daddr=\(spot.coordinate.latitude),\(spot.coordinate.longitude)&q=\(q)")
