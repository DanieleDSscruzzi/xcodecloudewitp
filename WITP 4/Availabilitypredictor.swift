...
final class AvailabilityPredictor {
    ...
    func predict(for spot: ParkingSpot, at date: Date = Date()) -> AvailabilityPrediction {
        ...
        // Combina: peso maggiore alla probabilità del singolo stallo (più
        // prudente). Il bonus "almeno-uno-libero" resta ma non domina:
        // prima gonfiava la stima e prometteva posti che non c'erano.
        prob = prob * 0.60 + probAtLeastOneFree * 0.40
        ...
        // Prudenza: la teoria sovrastima sempre (doppie file, soste
        // lampo, dati parziali). Un taglio onesto avvicina la stima a ciò
        // che l'utente vede coi propri occhi, invece di prometterlo pieno.
        prob *= 0.82
        // Tetto realistico: mai "sicuro al 99%" — è una previsione.
    ...
    private func hourlyOccupancyCurve(category: ZoneCategory, time t: Double, weekday: Int) -> Double {
        ...
        case .residential:
            ...
            if isWeekend {
                // Weekend: quasi tutti a casa, auto ferme → molto pieno
                return interpolate(t, points: [
                    (0, 0.94), (6, 0.94), (10, 0.88), (14, 0.85),
                    (18, 0.92), (22, 0.96), (24, 0.94)
                ])
            } else {
                // Feriale: anche di giorno resta parecchia gente; il "vuoto"
                // di metà giornata è modesto — sotto casa trovi posto di rado.
                return interpolate(t, points: [
                    (0, 0.96), (6, 0.95), (8, 0.80),  // 8: qualcuno esce
                    (12, 0.74), (14, 0.78),
                    (18, 0.90), (20, 0.95), (24, 0.96)
                ])
            }
